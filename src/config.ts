...
export function loadConfig(options?: Partial<SESMailerConfig>): SESMailerConfig {
  ...
  const analyticsApiKey =
    options?.analyticsApiKey ?? process.env.ANALYTICS_API_KEY ?? fileConfig.analyticsApiKey;
  const analyticsPort =
    options?.analyticsPort ??
    (Number(process.env.ANALYTICS_PORT) || undefined) ??
    fileConfig.analyticsPort;
  const maskEmails =
    options?.maskEmails ??
  ...
  const port =
    options?.smtp?.port ?? (Number(process.env.SMTP_PORT) || undefined) ?? fileConfig.smtp?.port;
  ...
  const rateLimit =
    options?.rateLimit ??
    (Number(process.env.SES_RATE_LIMIT) || undefined) ??
